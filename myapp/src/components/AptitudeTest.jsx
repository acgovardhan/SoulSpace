...
// These will be fetched from your MongoDB; below is a fallback / structure demo.
const FALLBACK_QUESTIONS = [
  {
    _id: "1",
    section: "Verbal Reasoning",
    question: "Choose the word that is most opposite in meaning to: TENACIOUS",
    options: ["A. Stubborn", "B. Yielding", "C. Brave", "D. Careless"],
    answer: "B",
  },
  {
    _id: "2",
    ...
    options: ["A. Canvas", "B. Artist", "C. Gallery", "D. Brush"],
    answer: "C",
  },
  {
    _id: "3",
    section: "Numerical Reasoning",
    question: "If a train travels 240 km in 3 hours, how far will it travel in 5 hours?",
    options: ["A. 360 km", "B. 400 km", "C. 420 km", "D. 480 km"],
    answer: "B",
  },
  {
    _id: "4",
  ...
  },
  {
    ...
    question: "In a pattern where shapes double each step: Circle → 2 Circles → 4 Circles, how many will be in step 5?",
  ...
  },
  {
    _id: "6",
    section: "Abstract Reasoning",
    question: "Which of the following comes next if the rule is: +2, ×2, +2, ×2 starting from 1?",
  ...
  },
  {
    ...
    question: "If Gear A (10 teeth) drives Gear B (20 teeth), and Gear A turns at 60 rpm, what is Gear B's speed?",
    options: ["A. 120 rpm", "B. 60 rpm", "C. 30 rpm", "D. 15 rpm"],
    answer: "C",
  },
  {
    _id: "9",
...
const AptitudeTest = ({ onBack, user }) => {
  ...
  // Fetch questions from DB, fallback to local
  useEffect(() => {
  ...
  if (submitted) {
    const correct = questions.filter(
      (q) => answers[q._id] === q.answer
    ).length;
    const pct = Math.round((correct / questions.length) * 100);
    const grade =
      pct >= 80
        ? { label: "Excellent", color: "#4caf80" }
        : pct >= 60
        ? { label: "Good", color: "#ffc107" }
        : pct >= 40
        ? { label: "Average", color: "#ff9800" }
      ...
      <div className="test-result-page">
        ...
        <div className="result-card">
          <div className="result-score-ring" style={{ borderColor: grade.color }}>
            <span className="result-score-num" style={{ color: grade.color }}>
              {pct}%
            </span>
          </div>
                ...
                <div key={sec} className="result-bar-row">
                  ...
                  <div className="result-bar-track">
                    <div
                      ...
                      style={{
                        width: `${(secCorrect / secQs.length) * 100}%`,
                        background: grade.color,
                      }}
                    />
                  </div>
                  <span className="result-bar-max">
          ...
          <div className="apt-review">
            ...
            {questions.map((q, i) => {
              ...
              return (
                <div
                  key={q._id}
                  className={`apt-review-item ${isCorrect ? "correct" : "wrong"}`}
                >
                  <p className="apt-review-q">
                    {i + 1}. {q.question}
                  </p>
                  <p>
                    Your answer:{" "}
                    <strong>{answers[q._id] || "Not answered"}</strong>
                    {!isCorrect && (
                      <span className="apt-correct-ans">
                        {" "}
          ...
          <div className="result-actions">
            {!saved ? (
              <button
                ...
                onClick={() => handleSave(correct, pct)}
                disabled={saving}
              >
              ...
              return (
                <div key={q._id} className="test-question-card">
                  <p className="test-q-text">
                    <span className="test-q-num">{globalIdx + 1}.</span>{" "}
                    {q.question}
                  </p>
                  <div className="test-options">
                    {q.options.map((opt) => {
                      ...
                      return (
                        <label
                          key={opt}
                          className={`test-option ${answers[q._id] === letter ? "selected" : ""}`}
                        >
                          <input
                            type="radio"
                            name={`q_${q._id}`}
