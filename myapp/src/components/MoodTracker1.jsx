...
const MoodTracker = ({ user }) => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const res = await fetch(`/api/mood?userId=${user?._id}`);
        if (res.ok) {
    ...
    <div className="mood-container">
      <div className="mood-header">
        ...
        <div className="mood-view-toggle">
          <button
            className={view === "log" ? "btn-primary" : "btn-muted"}
            onClick={() => setView("log")}
          >
            📝 Log Mood
          </button>
          <button
            className={view === "history" ? "btn-primary" : "btn-muted"}
        ...
        <div className="mood-log-card">
          ...
          {!gender && (
            <div className="mood-gender-select">
              ...
              <div className="mood-gender-btns">
                ...
                <button className="btn-muted" onClick={() => setGender("male")}>
                  👨 Male
                </button>
                <button
          ...
          {gender && (
            <>
              ...
              <div className="mood-emoji-row">
                {EMOJI_MOODS.map((m) => (
                  ...
                  >
                    ...
                    <span className="mood-label" style={{ color: m.color }}>
                      {m.label}
                    </span>
                  </button>
                ))}
              ...
              {/* Menstrual cycle phase – females only */}
              {gender === "female" && (
              ...
              <div className="mood-save-row">
                ...
                {saved && (
                  <span className="mood-saved-msg">✅ Mood logged!</span>
                )}
                <button
                  className="btn-muted"
                  style={{ marginLeft: 8 }}
        ...
        <div className="mood-history">
          {history.length === 0 ? (
            <div className="mood-empty">
              ...
              <button className="btn-primary" onClick={() => setView("log")}>
                Log Your First Mood
              </button>
            </div>
          ) : (
            <>
              ...
              <div className="mood-chart">
                ...
                <div className="mood-bar-chart">
                  {history.slice(-7).map((entry, i) => {
                    const mood = EMOJI_MOODS.find(
                      (m) => m.value === entry.mood?.value
                    );
                    return (
              ...
              <div className="mood-list">
                {[...history].reverse().map((entry, i) => {
                  ...
                  const phase = CYCLE_PHASES.find(
                    (p) => p.id === entry.phase
                  );
                  return (
                    <div key={i} className="mood-list-item">
                      <span className="mood-list-emoji">
                        {mood?.emoji || "😐"}
                      </span>
                      <div className="mood-list-info">
                        <strong style={{ color: mood?.color }}>
                          {mood?.label}
                        </strong>
                        {phase && (
                          <span className="mood-list-phase">
                            {" "}
                            · {phase.icon} {phase.label}
                          </span>
                        )}
                        {entry.note && (
                          <p className="mood-list-note">{entry.note}</p>
                        )}
                      </div>
